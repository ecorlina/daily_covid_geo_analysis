...
# import count data from the Excel file and merge with geo data ----
...
for (i in n_days:1) {
   ...
   map_layer_counts <-
      ...
      tm_fill(col = "count",
              ...
              title = "Confirmed Cases",
              group = str_c("COVID-19 case count, ", this_sheet),
              legend.show = ifelse(i == n_days, T, F)) +
      tm_borders()
   assign(str_c("basemap_", i), map_layer_counts)
   map_layer_rates <-
      tm_shape(ndsc_map_counts) +
      tm_fill(col = "rate_comp",
              style = "fixed",
              breaks = c(0, 10, 32, 100, 316, 1000, 3162),
              title = "Confirmed Case Rates (per 100k)",
              group = str_c("COVID-19 case rate, ", this_sheet),
              legend.show = ifelse(i == n_days, T, F)) +
      tm_borders()
   assign(str_c("basemap_rates_", i), map_layer_rates)
}

# this line is only here to pause catch the processor UI before it unfolds the next block...
...
static_count_map_binary <- la_county_topomap + the_count_map_binary_by_day
...
static_rate_map_binary <- la_county_topomap + the_rate_map_binary_by_day
